    ...
    available_tastes.each do |taste|
      ...
      compilation_script = <<~RUBY
        ...
        #{'  '}
          ...
          $stderr = original_stderr if defined?(original_stderr)
          puts "FAILED: #{taste} - Error: \#{e.class}: \#{e.message}"
          puts "Backtrace:"
      ...
      begin
        # Execute the script in a subprocess and capture output
        output = `ruby #{script_file} 2>&1`
        result = $?.success?

        # Always print the output from the subprocess
        ...
        if result
          ...
          puts "\n" + "=" * 80
          puts "DETAILED ERROR OUTPUT FOR #{taste.upcase}:"
          puts "=" * 80
          puts output
          puts "=" * 80
